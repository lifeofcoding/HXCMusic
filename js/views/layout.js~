...
/* 				this.messi.addClass('magictime puffOut'); */
		...
		updateQueue: function() {
			...
			//$('#current_playlist_rows').html(_.template($('#queue-tpl').html())());
		},
		...
		addAllToQueue: function() {
			if (App.currentPage !== 'searchVideos') {
				...
				if (collection.models.length > 0) {
					//App.Model.Queue.add(collection.models);
		...
		updateUILoggedIn: function() {
			var me = this;
			//App.Util.waitTillReady('#top_right', function() {
			var updateUI = function() {
				...
				//me.UserNav.$el.hide();
				//setTimeout(function(){ me.UserNav.$el.addClass('animated fadeInUp').show(); }, 3000);
...
/* 			var css = {},
				homepage = $('#homepage');
			css.transition = '-webkit-transform 1s';
			css[App.browserPrefix.css + 'transform'] = 'translate3d(0, 100%,0)';
			homepage.css(css).afterTransition(function() {
...
/* 				open: function(){
					var me = $(this);
					me.hide();
					me.animo({ animation: 'vanishIn', duration: 0.35, keep: false });
				}, */
				openDuration: 200,
				closeDuration: 200,
				open: function(){
				...
				beforeOpen: function() {
					var me = $(this);
					//me.hide();
					//me.animo({ animation: 'jelly', duration: 1, keep: false });
					if (title === '') {
		...
		showError: function() {
			...
			$('#explore_content_sotd').html('<div class="error-wrapper"><div class="error-message">Oops... Looks like we are having some trouble.</div><br>We have been notified of the problem. Please wait a few minutes and try again.</div>');
		},
		...
		showLoading: function(title) {
			...
			//$('#explore_content_sotd').html('<center><img src="' + App.baseURL + '/images/loader-white.gif" id="content-loader"></center>');
			...
			/*$('#fullpage-loading').animo({ animation: 'puffIn', duration: 0.4, keep: false }, function(){
				$('#fullpage-loading').css({display:'inline-block'});
			}).show();*/
			$('#spinner').fadeIn();
			$('#middle').addClass('blur');
			//$('#fullpage-loading').fadeIn();
			//this.ui.loader.fadeIn();
		},
		...
		hideLoading: function() {
			...
			//$('#content-loader').fadeOut('normal', function() {
			//	$(this).remove();
			//});
			//$('#fullpage-loading').fadeOut();
			$('#spinner').fadeOut();
			$('#middle').removeClass('blur');
			/*$('#fullpage-loading').animo({ animation: 'puffOut', duration: 0.5, keep: false }, function(){
				$('#fullpage-loading').hide();
			});*/
/* 			setTimeout(function(){
				$('#fullpage-loading').animo({ animation: 'puffOut', duration: 0.5, keep: false }, function(){
					var newImage = '<div id="fullpage-loading"><img src="/loading-bars.svg"></div>';
					$('#fullpage-loading').remove();
					$('body').prepend(newImage);
					//$('#fullpage-loading').removeAttr('style').removeClass('animated puffin').hide();
				});
			}, 2000); */
		},
		...
		showResults: function() {
			...
			this.displayAd();
			//$('#middle').backstretch(App.baseURL + "/images/universe.png", {fade: 1000, centeredX: false, centeredY: true});
		}
	});
