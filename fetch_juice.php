...
include("/var/zpanel/hostdata/zadmin/public_html/crypt.php");
...
if(!isset($_GET['geturl'])){
  ...
  $url = 'http://mp3juices.com/search/'.$srch_url;
  //echo $url;
  //die;
  $ch  = curl_init();
  ...
  $res = strstr($gurl, '<td style="width:240px;min-width:240px" class="controls">');
...
if (!$res) {
    ...
    $url = 'http://hxcmusic.me/youtube-api/examples/youtube/search.php?q=' . $srch . '&maxResults=' . $maxresults . '';
...
if ($res) {
    ...
    while ($i <= $lapse) {
        ...
        $res = strstr($res, '<td style="width:240px;min-width:240px" class="controls">');
        
/*         $artist = extstr3($res, '<artist>', '</artist>');
        $title  = extstr3($res, '<title>', '</title>'); */
        $name   = extstr3($res, '<h2 class="song_title" style="display:inline!important;height:auto!important;font-size:13pt;font-weight:400;padding:0;margin:0;">', '</h2>');
...
/*         $getid1 = extstr3($res, "return operate(", ","); */
        $id     = extstr3($res, '<input type="hidden" name="files['.($i + 1).'][url]" value="', '"');
		
		$quality = extstr3($res, '<input type="hidden" name="files['.($i + 1).'][bitrate]" value="', '"');
		
		$length = extstr3($res, '<input type="hidden" name="files['.($i + 1).'][len]" value="', '"');
        ...
        $encoded = str_replace('.mp3', '', $encoded);
        //$track = dec($encoded);
        //$track = Encode(hex2bin($encoded),$secretPass);
        ...
        if ((strpos($matcha, $matchb) !== false)&&($quality >= 100)&&($length > 100)&&(!$isLive)&&(!$isCover)) {
            //echo 'Found!';
            //die;
            ...
            if ($limit == '1') {
                ...
                header("Content-Disposition: attachment; filename=" . $filename . "[HXCMusic.me].mp3");
        ...
        $dur = extstr3($res, 'Duration: ', ' &ndash;');
...
$url = "http://hxcmusic.me/engine/" . $srch . "/1/mp3";
